		...
		if ( $user ) {
			...
			if ( affiliate_is_plugin_active_for_network() ) {
				if ( current_user_can( 'manage_network_options' ) ) {
					?><p><?php _e( 'Affiliate User', 'affiliate' ) ?> <a
...
function aff_mp_addon_add_metabox() {
	$metabox = new WPMUDEV_Metabox( array(
		...
		'title'       => __( 'Affiliate Settings', 'mp' ),
		'option_name' => 'affiliate_mp_percentage',
		'order'       => 99,
	) );
	$metabox->add_field( 'text', array(
		'name'        => 'aff_commission',
		'label'       => array( 'text' => __( 'Set percentage to be paid to affiliates', 'mp' ) ),
		'desc'        => __( "You can set the global commision amount paid to affiliates for referred purchases below. Set it to 0 for no payments.", 'mp' ),
		'after_field' => '%',
		'style'       => 'width:150px',
		'validation'  => array(
			'number' => true,
...
function affiliate_marketpress_display_metabox( $order ) {
	...
	if ( ( isset( $order->mp_shipping_info['affiliate_referrer'] ) ) && ( ! empty( $order->mp_shipping_info['affiliate_referrer'] ) ) ) {
		...
		<div id="mp-order-notes-affiliate" class="postbox">
			<h3 class='hndle'><span><?php _e( 'Affiliate', 'affiliate' ); ?></span> - <span
					class="description"><?php _e( 'This order was received via an Affiliate Referrer link.', 'affiliate' ); ?></span>
			</h3>
			<div class="inside">
				<?php
				//echo "order status[". $order->post_status ."]<br />";
...
function affiliate_marketpress_settings( $settings ) {
	...
	<div id="mp_gateways" class="postbox">
		...
		<div class="inside">
			<span
				class="description"><?php _e( 'You can set the global commision amount paid to affiliates for referred purchases below. Set it to 0 for no payments.', 'affiliate' ); ?></span>
